...
async def rss_feed():
    ...
    build_date = format_datetime(latest_date, usegmt=True) if latest_date else format_datetime(datetime.now(timezone.utc), usegmt=True)
...
async def sitemap():
    conn = get_db()
    posts = conn.execute("SELECT slug, updated_at FROM posts WHERE status='active' AND category NOT IN ('AI Tool','Project','Indie Dev','Site') ORDER BY updated_at DESC").fetchall()
    ...
    # BUGFIX: was incorrectly pointing to /blog/ instead of /indie-devs/
    for d in indie_devs:
        url_entry("https://just4.tech/blog", d["slug"], d["updated_at"], "0.5")  # indie devs live at /blog/{slug}
