  ...
  <div>
    <section class="hero-gradient py-20">
      <div class="max-w-4xl mx-auto px-4 text-center relative z-10">
        ...
        <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4">{{ page.title || 'Terms of Use' }}</h1>
...
onMounted(async () => {
  try {
    const resp = await fetch('/api/page/terms')
    if (resp.ok) page.value = await resp.json()
  } catch {}
  loading.value = false
  usePageMeta({
    title: page.value.meta_title || 'Terms of Use',
    description: page.value.meta_description || 'Terms of Use for Just4Tech.'
  })
})
</script>
