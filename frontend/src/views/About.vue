    ...
    <section class="hero-gradient py-20">
      <div class="max-w-4xl mx-auto px-4 text-center relative z-10">
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 text-amber-400 text-sm font-medium mb-6 border border-white/10">
          ...
          <span>{{ page.title || 'About' }}</span>
        </div>
        <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4">{{ page.title || 'About Just4Tech' }}</h1>
        <p v-if="page.meta_description" class="text-lg text-gray-400 max-w-2xl mx-auto">{{ page.meta_description }}</p>
      </div>
    </section>
...
onMounted(async () => {
  try {
    const resp = await fetch('/api/page/about')
    if (resp.ok) page.value = await resp.json()
  } catch {}
  loading.value = false
  usePageMeta({
    title: page.value.meta_title || page.value.title || 'About',
    description: page.value.meta_description || 'About Just4Tech.'
  })
})
</script>
